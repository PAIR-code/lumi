...
from shared.lumi_doc import LumiDoc, LumiSection, LumiContent, LumiSpan, Label, Heading, TextContent, ListContent, ListItem
...
class PromptUtilsTest(unittest.TestCase):
    def test_get_json_from_response(self):
        ...
        with self.subTest(name="json_list"):
            mock_response = ModelResponse(model_output=['```json[{"id": "1", "label": "A"}, {"id": "2", "label": "B"}]```'])
    ...
    def test_get_labels_from_response(self):
        ...
        with self.subTest(name="valid_labels"):
            mock_json_output = '[{"id": "s1", "label": "summary1"}, {"id": "s2", "label": "summary2"}]'
            mock_response = ModelResponse(model_output=[f'```json{mock_json_output}```'])
            expected_labels = [
    ...
    def test_get_all_spans_from_doc(self):
        ...
        with self.subTest(name="extract_all"):
            ...
            self.assertEqual(len(extracted_spans), 4)
